...
class Utils {
    ...
    public static function thumbnailDirectory() {
        return self::buildPath(array(self::rootDirectory(), "thumbnails"));
    }
    ...
    public static function buildPath($pathElements) {
        $path = "";
        $size = count($pathElements);
        foreach ($pathElements as $index => $element) {
            $path = $path . $element;
            if ($index < $size - 1) {
                $path = $path . self::directorySeparator();
            }
        }
        return $path;
    }
